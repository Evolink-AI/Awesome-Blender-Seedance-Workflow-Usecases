...
def check(path: Path) -> Result:
    ...
    try:
        with urllib.request.urlopen(request, timeout=30) as response:
            return Result(path, url, response.status, response.headers.get_content_type())
    except urllib.error.HTTPError as exc:
        return Result(path, url, exc.code, exc.headers.get_content_type() if exc.headers else "", str(exc))
    except Exception as exc:
        return Result(path, url, 0, "", str(exc))
...
def main() -> None:
    ...
    for result in results:
        audit.append(f"| `{result.path.relative_to(ROOT).as_posix()}` | {result.status} | `{result.content_type}` | {result.url} |")
    ...
    posters = [
        ...
        "",
        f"- Generated: {datetime.now(timezone.utc).isoformat()}",
        f"- Videos: {len(list(MEDIA.glob('*.mp4')))}",
    ...
    posters.insert(5, f"- Result: `{'PASS' if not poster_failures else 'FAIL'}`")
    ...
    print(f"PASS: {len(results)} R2 objects and {len(list(MEDIA.glob('*.mp4')))} video/poster pairs")
